...
- **실측 단위**: **125 px = 1 m** (`src/lib/units.ts`, Oceanis 473 실측 선체 14.30 m·빔 4.46 m 캘리브레이션).
  API 좌표/크기는 px — 미터로 계산하려면 `÷125`. 예: 2 m 폭 로커 = `w: 250`.
  UI: 편집 툴바 📏 측정 도구(드래그 거리 m 표시), 도형 속성의 "실측 크기(m)" 입력, 도면 좌하단 축척 바(2 m).
